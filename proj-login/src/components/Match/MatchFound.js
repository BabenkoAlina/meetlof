...
function MatchFound({ onLogout }) {
  ...
  const handleLikeClick = async () => {
    if (currentUser && matchedUsers.length > currentMatchIndex) {
      ...
      await updateDoc(userDocRef, {
        [`history.${matchedUsers[currentMatchIndex].userId}`]: matchTime,
      });
  ...
  const handleSkipClick = async () => {
    if (currentUser && matchedUsers.length > currentMatchIndex) {
      const userDocRef = doc(db, "usersHistory", currentUser.uid);
      await updateDoc(userDocRef, {
        rejectedList: arrayUnion(matchedUsers[currentMatchIndex].userId),
      });
      setCurrentMatchIndex((prevIndex) => prevIndex + 1);
    }
  };
  ...
  return (
    <div>
      ...
      {matchedUsers.length > 0 && matchedUsers.length > currentMatchIndex ? (
        <div className={styles.match_card}>
          ...
          <div className={styles.match_card_top}>
            <div className={styles.match_card_top_right}>
              ...
              <p>{matchedUsers[currentMatchIndex]?.data?.expectation}</p>
              {matchedUsers[currentMatchIndex]?.data?.profileImage && (
                <img
                  src={matchedUsers[currentMatchIndex]?.data?.profileImage}
                  alt="Matched User"
          ...
          <button className={styles.exit} onClick={() => history.push("/home")}>
            Exit
          </button>
        </div>
      ) : (
